...
func main() {
	...
	safeCount := 0
	for s.Scan() {
		if checkSafeLevels(parseLevels(s.Text())) {
			safeCount++
		}
	}
...
func checkSafeLevels(levels []int) bool {
	...
	for i := 0; i < len(levels); i++ {
		levelsExcludingCurrent := getLevelsExcludingIndex(levels, i)
		if areSafeLevels(levelsExcludingCurrent) {
			return true
		}
	}
	return false
...
func getLevelsExcludingIndex(levels []int, i int) []int {
